...
export const AIChatDemo: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4" aria-labelledby="chat-heading">
      <div className="text-center mb-10">
        <h3 id="chat-heading" className="text-3xl font-bold text-white mb-2">NexusAI Concierge</h3>
      ...
      <div className="glass-morphism rounded-[2rem] overflow-hidden border border-white/10 shadow-2xl flex flex-col h-[600px]">
        ...
        >
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-6 opacity-60">
              ...
              <div>
                <p className="text-lg font-medium text-white mb-4">How can I help you today?</p>
                <div className="flex flex-wrap justify-center gap-2 max-w-sm">
                  {suggestions.map((s) => (
                    <button 
                      ...
                      aria-label={`Ask: ${s}`}
                      className="px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full text-xs text-gray-400 hover:text-white transition-all active:scale-95"
                    >
                      {s}
                    </button>
                  ))}
            ...
            >
              ...
              >
                <div className="whitespace-pre-wrap">
                  {msg.text || (msg.role === 'model' && <span className="flex space-x-1" aria-label="Thinking..."><span className="w-1.5 h-1.5 bg-gray-500 rounded-full animate-bounce"></span><span className="w-1.5 h-1.5 bg-gray-500 rounded-full animate-bounce [animation-delay:0.2s]"></span><span className="w-1.5 h-1.5 bg-gray-500 rounded-full animate-bounce [animation-delay:0.4s]"></span></span>)}
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="p-6 bg-white/5 border-t border-white/5">
          ...
          >
            <label htmlFor="ai-input" className="sr-only">Message NexusAI</label>
            <input 
              id="ai-input"
              type="text"
              value={input}
            ...
            <button 
              ...
              className="absolute right-2 top-2 bottom-2 px-4 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40 rounded-xl text-white transition-all active:scale-95 flex items-center justify-center"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          <p className="mt-3 text-[10px] text-gray-600 text-center uppercase tracking-widest font-bold">
            Powered by Gemini 3 Flash • Intelligent Strategic Analysis
          </p>
        </div>
      </div>
    </div>
